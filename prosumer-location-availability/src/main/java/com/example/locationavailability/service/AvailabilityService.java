...
public class AvailabilityService {
    ...
    public LocationAvailabilityResponse checkAvailabilitySync(String location, String date) {
        try {
            ...
            CompletableFuture<List<TrailAvailability>> trailsFuture = 
                CompletableFuture.supplyAsync(() -> 
                    trailServiceClient.getTrailsByLocation(location)
                );
            
            CompletableFuture<WeatherInfo> weatherFuture = 
                CompletableFuture.supplyAsync(() -> 
                    weatherServiceClient.getWeatherForecast(location, date)
                );
    ...
    private LocationAvailabilityResponse buildResponse(
        ...
        // Calcola disponibilità complessiva
        boolean overall = trails.stream().anyMatch(TrailAvailability::isAvailable) && 
                          weather.isSuitableForTrails();
        ...
        response.setOverallAvailability(overall);
